...
function volumerhs!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, ::direction,
  ...
  l_Qhypervisc = MArray{Tuple{3nhypergradstate}, FT}(undef)
    ...
    @loop for k in (1:Nqk; threadIdx().z)
      @loop for j in (1:Nq; threadIdx().y)
        @loop for i in (1:Nq; threadIdx().x)
          ...
          @unroll for s = 1:3nhypergradstate
...
function facerhs!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, ::direction,
  ...
  l_QhyperviscM = MArray{Tuple{3nhypergradstate}, FT}(undef)
  ...
  l_QhyperviscP = MArray{Tuple{3nhypergradstate}, FT}(undef)
...
function volumehyperviscterms_a_strong!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
  ...
  s_G = @shmem FT (Nq, Nq, Nqk, nhypergradstate, 3)
  s_D = @shmem FT (Nq, Nq)
    ...
    @loop for k in (1:Nqk; threadIdx().z)
      @loop for j in (1:Nq; threadIdx().y)
        @loop for i in (1:Nq; threadIdx().x)
          ijk = i + Nq * ((j-1) + Nq * (k-1))
          @unroll for s = 1:nhypergradstate
            s_G[i, j, k, s, 1] = Qhypervisc_grad[ijk, 3(s - 1) + 1, e]
            s_G[i, j, k, s, 2] = Qhypervisc_grad[ijk, 3(s - 1) + 2, e]
            s_G[i, j, k, s, 3] = Qhypervisc_grad[ijk, 3(s - 1) + 3, e]
          end
        end
      end
    end
      ...
      @loop for j in (1:Nq; threadIdx().y)
        @loop for i in (1:Nq; threadIdx().x)
          ...
          @unroll for s = 1:nhypergradstate
            ...
            @unroll for n = 1:Nq
              G1ξ1 += s_D[i, n] * s_G[n, j, k, s, 1]
              G2ξ1 += s_D[i, n] * s_G[n, j, k, s, 2]
              G3ξ1 += s_D[i, n] * s_G[n, j, k, s, 3]
              if dim == 3 || (dim == 2 && direction == EveryDirection)
                G1ξ2 += s_D[j, n] * s_G[i, n, k, s, 1]
                G2ξ2 += s_D[j, n] * s_G[i, n, k, s, 2]
                G2ξ3 += s_D[j, n] * s_G[i, n, k, s, 3]
              end
              if dim == 3 && direction == EveryDirection
                G1ξ3 += s_D[k, n] * s_G[i, j, n, s, 1]
                G2ξ3 += s_D[k, n] * s_G[i, j, n, s, 2]
                G3ξ3 += s_D[k, n] * s_G[i, j, n, s, 3]
              end
            end
    ...
    @loop for k in (1:Nqk; threadIdx().z)
      @loop for j in (1:Nq; threadIdx().y)
        @loop for i in (1:Nq; threadIdx().x)
          ...
          @unroll for s = 1:nhypergradstate
            Gξ1 = Gξ2 = Gξ3 = zero(FT)
            Gξ1x1 = Gξ2x1 = Gξ3x1 = zero(FT)
            Gξ1x2 = Gξ2x2 = Gξ3x2 = zero(FT)
            Gξ1x3 = Gξ2x3 = Gξ3x3 = zero(FT)
            @unroll for n = 1:Nq
              Gξ1 += s_D[i, n] * s_G[n, j, k, s]
              njk = n + Nq * ((j-1) + Nq * (k-1))
              M = vgeo[njk, _M, e]
              ...
              Gξ1x1 += ξ1x1 * M * s_D[n, i] * s_G[n, j, k, s]
              Gξ1x2 += ξ1x2 * M * s_D[n, i] * s_G[n, j, k, s]
              Gξ1x3 += ξ1x3 * M * s_D[n, i] * s_G[n, j, k, s]
              if dim == 3 || (dim == 2 && direction == EveryDirection)
                #Gξ2 += s_D[j, n] * s_G[i, n, k, s]
                ink = i + Nq * ((n-1) + Nq * (k-1))
                M = vgeo[ink, _M, e]
                ...
                Gξ2x1 += ξ2x1 * M * s_D[n, j] * s_G[i, n, k, s]
                Gξ2x2 += ξ2x2 * M * s_D[n, j] * s_G[i, n, k, s]
                Gξ2x3 += ξ2x3 * M * s_D[n, j] * s_G[i, n, k, s]
              end
              if dim == 3 && direction == EveryDirection
                #Gξ3 += s_D[k, n] * s_G[i, j, n, s]
                ijn = i + Nq * ((j-1) + Nq * (n-1))
                ...
                Gξ3x1 += ξ3x1 * M * s_D[n, k] * s_G[i, j, n, s]
                Gξ3x2 += ξ3x2 * M * s_D[n, k] * s_G[i, j, n, s]
                Gξ3x3 += ξ3x3 * M * s_D[n, k] * s_G[i, j, n, s]
              end
            end
            #l_gradG[1, s] = ξ1x1 * Gξ1
            #l_gradG[2, s] = ξ1x2 * Gξ1
            ...
            MI = vgeo[ijk, _MI, e]
            l_gradG[1, s] = -MI * Gξ1x1
            l_gradG[2, s] = -MI * Gξ1x2
            l_gradG[3, s] = -MI * Gξ1x3
            ...
            if dim == 3 || (dim == 2 && direction == EveryDirection)
              ...
              l_gradG[1, s] -= MI * Gξ2x1
              l_gradG[2, s] -= MI * Gξ2x2
              l_gradG[3, s] -= MI * Gξ2x3
            end
            ...
            if dim == 3 && direction == EveryDirection
              ...
              l_gradG[1, s] -= MI * Gξ3x1
              l_gradG[2, s] -= MI * Gξ3x2
              l_gradG[3, s] -= MI * Gξ3x3
            end
          end
          ...
          hyperdiffusive!(bl,
                          Vars{vars_hyperdiffusive(bl,FT)}(l_Qhypervisc),
                          Grad{vars_hypergradient(bl,FT)}(l_gradG),
                          Vars{vars_state(bl,FT)}(l_Q[:, i, j, k]),
...
function facehyperviscterms_b_weak!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
  ...
  l_QlapM = MArray{Tuple{nhypergradstate}, FT}(undef)
  l_QlapP = MArray{Tuple{nhypergradstate}, FT}(undef)
  l_Qhypervisc = MArray{Tuple{nhyperviscstate}, FT}(undef)
  ...
  @inbounds @loop for e in (elems; blockIdx().x)
    for f in faces
      @loop for n in (1:Nfp; threadIdx().x)
        ...
        @unroll for s = 1:nhypergradstate
          l_QlapM[s] = Qhypervisc_div[vidM, s, eM]
        end
        ...
        @unroll for s = 1:nhypergradstate
          l_QlapP[s] = Qhypervisc_div[vidP, s, eP]
        end
        ...
        if bctype == 0
          #hypergrad_penalty!(hypergradnumpenalty, bl, l_gradG,
          #                   nM, l_QlapM, l_QlapP)
          hypergrad_penalty!(hypergradnumpenalty, bl, l_Qhypervisc,
                             nM, l_QlapM, l_QM, l_auxM, l_QlapP, l_QP, l_auxP, t)
        else
          #hypergrad_boundary_penalty!(hypergradnumpenalty, bl, l_gradG, nM, l_GM,
          #                            l_QM, l_auxM, l_GP, l_QP, l_auxP, bctype,
          #                            t, l_Q_bot1, l_aux_bot1)
